...
const createCart = async function (req, res) {
  ...
  try {
    ...
    if (product.installments === 0) {
      return res.status(200).send({ status: false, message: `Product Is Out Of Stock Currently.` });
    }
    ...
    if (!req.body.hasOwnProperty("cartId")) {
      res.status(400).send({ status: false, message: `The Cart Is Aleady Present for ${userId} userId,Please Enter  corresponding CartID` })
      return
    }

    if (isCartExist._id != cartId) {
      res.status(400).send({ Status: false, message: "Cart Id and user do not match" })
      return
    }
...
const updateCart = async (req, res) => {
  ...
  try {
    ...
    if (!isCartExist) {
      res.status(404).send({ status: false, message: `Cart Not Found Please Check Cart Id` })
      return
    }
    ...
    if (index == -1) {
      res.status(400).send({ status: false, message: `Product Does Not Exist In Cart` });
      return
    }
...
const deleteCart = async function (req, res) {
  ...
  try {
    ...
    if (id == req.userId) {
      ...
      res.status(403).send({ status: false, message: "User not authorized to delete cart" });
    }
